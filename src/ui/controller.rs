...
pub fn fps_controller_input(
    ...
    for (controller, mut input) in query.iter_mut() {
        ...
        let to_local_up = Quat::from_rotation_arc(controller.local_up, Vec3::Y);
        ...
        input.movement = to_local_up * Vec3::new(
...
pub fn fps_controller_move(
    ...
    for (entity, input, mut controller, mut collider, mut transform, mut velocity) in query.iter_mut() {
        ...
        let to_local_up = Quat::from_rotation_arc(controller.local_up, Vec3::Y);
        velocity.linvel = to_local_up * velocity.linvel;
        ...
        let to_world_up = Quat::from_rotation_arc(Vec3::Y, controller.local_up);
        velocity.linvel = to_world_up * velocity.linvel;
    }
}
